...
def initparse():
    ...
    parser.add_argument(
        "filename", type=str, help="The name of the file to be used by the server application")
    parser.add_argument("hostnamePort", type=str,
                        help="The target port for the client to connect to")
    parser.add_argument("cipher", type=str, choices=[
                        "aes256", "aes128", "none"], help="The cipher to be used for communicaiton with the server")
    parser.add_argument(
        "key", nargs='?', help="The secret key to be used by the symmetric ciphers")
...
def main():
    ...
    # Set up the Ciphers if requested
        ...
        if (args.cipher == "aes128"):
            cipher = Cipher(algorithms.AES(key[0:16].encode(
                "utf-8")), modes.CBC(iv), backend=backend)  # Only want first half of the key
    ...
    if (cipher != None):
        try:
            response = EasyCrypto.decryptAndUnpad(cipher, soc.recv(
                256)).decode("utf-8")  # recieve server response
        except ValueError:
            eprint(datetime.now(), "Key doesn't match. Closing connection.")
            soc.close()
            return False
        # send key padded to server
        soc.send(EasyCrypto.encryptAndPad(cipher, key.encode("utf-8")))

    response = EasyCrypto.decryptAndUnpad(
        cipher, soc.recv(256)).decode("utf-8")
    if (response != "Cont"):
